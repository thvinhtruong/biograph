...
- **Scratchpad protection** — `<!-- biograph:scratchpad -->` is the hard delimiter in every ledger. On re-ingest, everything above is regenerated; everything below is preserved exactly.
...
```bash
biograph ingest lecture.pdf --course deep_learning --exam-date 2026-06-15
biograph ask "How does backpropagation use the chain rule?"
biograph ask "Explain attention" --course deep_learning --limit 15
biograph search "gradient descent" --limit 5
...
Data layer:
  biograph.db     — SQLite (nodes, FTS5 virtual table, query_log)
...
internal/cli/ingest.go            biograph ingest — flags, calls Pipeline.Run
internal/cli/ask.go               biograph ask — FTS5 → context assembly → LLM → print
internal/cli/search.go            biograph search — FTS5 results, formatted output
internal/cli/status.go            biograph status — prints DB.Stats
internal/cli/quiz.go              biograph quiz — multi-turn interactive exam session
...
The delimiter `<!-- biograph:scratchpad -->` divides every ledger:
...
Default model: `claude-haiku-4-5-20251001`. Change via `llm.model` in `biograph.yaml`.
...
- FTS5 sync triggers fire on SQLite INSERT/UPDATE/DELETE. Do not bypass them with direct `sqlite3` CLI edits or the FTS index will be stale. If it happens, delete `biograph.db` and re-ingest.
